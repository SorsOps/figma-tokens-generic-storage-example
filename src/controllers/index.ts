...
export class TokensController extends Controller {
    ...
    public async getTokens(
        ...
        if (!version) {
            // Get the latest
            response = await db.get('SELECT updatedAt, json, themes, pluginVersion FROM Tokens GROUP BY updatedAt  ORDER BY max(updatedAt) LIMIT 1; ');
        } else {
            //Attempt to retrieve a specific version
            response = await db.get('SELECT updatedAt, json, themes, pluginVersion FROM Tokens WHERE updatedAt = :updated; ', {
                ':updated': version
            });
        }
        ...
        return {
            ...
            $themes: JSON.parse(response.themes),
    ...
    public async postTokens(tokenSet: string,
        ...
        let created = false;
        let updatedAt = requestBody.updatedAt ? new Date(requestBody.updatedAt) : new Date();
        ...
        if (created) {
            ...
            await db.exec('CREATE Table Tokens (updatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, json TEXT, themes TEXT, pluginVersion varchar(255),PRIMARY KEY (updatedAt));');
